...
def test_extracts_bounded_canonical_text(make_docx, tmp_path):
    ...
    result = extract_document(io.BytesIO(source.read_bytes()), config=make_config(tmp_path))
...
def test_rejects_unsupported_package(tmp_path):
    ...
    with pytest.raises(DocumentExtractionError) as raised:
        extract_document(io.BytesIO(b"\xd0\xcf\x11\xe0" + b"\x00" * 64), config=config)
...
def test_rejects_malformed_package(tmp_path):
    config = make_config(tmp_path)
    with pytest.raises(DocumentExtractionError) as raised:
        extract_document(io.BytesIO(b"definitely not a docx at all"), config=config)
...
def test_production_fails_closed_without_linux_isolation(tmp_path, monkeypatch):
    ...
    with pytest.raises(DocumentExtractionError) as raised:
        extract_document(io.BytesIO(b"unused"), config=config)
...
def test_telemetry_never_contains_source_content(make_docx, tmp_path, caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        result = extract_document(io.BytesIO(secret_bytes), config=make_config(tmp_path))
...
def test_extracts_pdf_through_the_isolated_boundary(make_pdf, tmp_path):
    source = make_pdf(pages=["Jane Doe\nPlatform Engineer", "Built reliable systems."])
    result = extract_document(io.BytesIO(source.read_bytes()), config=make_config(tmp_path))
    assert "Platform Engineer" in result.text
    assert "Built reliable systems." in result.text
    assert result.code_points == len(result.text)


def test_rejects_encrypted_pdf_through_the_boundary(make_pdf, tmp_path):
    source = make_pdf(pages=["Secret"], encrypt="a-password")
    with pytest.raises(DocumentExtractionError) as raised:
        extract_document(io.BytesIO(source.read_bytes()), config=make_config(tmp_path))
    assert raised.value.category == "unsupported_format"


def test_rejects_image_only_pdf_through_the_boundary(make_pdf, tmp_path):
...
def test_rejects_pdf_over_page_budget_through_the_boundary(make_pdf, tmp_path):
    source = make_pdf(pages=[f"Page {index}" for index in range(6)])
    config = make_config(tmp_path, max_pdf_pages=5)
    with pytest.raises(DocumentExtractionError) as raised:
        extract_document(io.BytesIO(source.read_bytes()), config=config)
    assert raised.value.category == "over_budget"


def test_rejects_truncated_pdf_through_the_boundary(make_pdf, tmp_path):
    source = make_pdf(pages=["Jane Doe"])
    truncated = source.with_name("truncated.pdf")
    truncated.write_bytes(source.read_bytes()[: len(source.read_bytes()) // 2])
    with pytest.raises(DocumentExtractionError) as raised:
        extract_document(io.BytesIO(truncated.read_bytes()), config=make_config(tmp_path))
    assert raised.value.category == "malformed_document"


def test_removes_private_storage_on_pdf_success(make_pdf, tmp_path):
...
def test_pdf_telemetry_never_contains_source_content(make_pdf, tmp_path, caplog):
    source = make_pdf(pages=["SecretMarkerPdfBodyText"])
    secret_bytes = source.read_bytes()
    with caplog.at_level(logging.DEBUG):
        result = extract_document(io.BytesIO(secret_bytes), config=make_config(tmp_path))
    assert "SecretMarkerPdfBodyText" in result.text
    for record in caplog.records:
        assert "SecretMarkerPdfBodyText" not in record.getMessage()
        leaked = any("SecretMarkerPdfBodyText" in str(value) for value in record.__dict__.values())
        assert not leaked
